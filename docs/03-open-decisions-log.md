...
| 5 | The Absence Rules table, as already built in the live Airtable base, encodes **four** separate rule records with distinct alert-level thresholds (2, 5, 7, and 10 days via the Threshold / Alert Level fields) — not the PRD's single "4 qualifying absence days in a rolling 30-day period" rule. This is the same underlying conflict as row 1, now confirmed present in the actual table structure rather than just the two planning docs. The two models produce materially different alerting behaviour (one alert per intern per 30-day cycle vs. up to four escalating alerts), so this needs explicit sign-off on which model rolling-cycle-count.js and alert-case-and-email.js should implement before either is written. | Open | — | — | Phase 4 (Rolling-cycle and alerts) — do not start rolling-cycle-count.js until this is resolved | Source: docs/04-airtable-api-reference.md, "Tables & Table IDs" (Absence Rules: 4 records) and "Absence Rules" field list (Threshold, Alert Level), verified against the live base via verify-airtable-connection.js on 2026-09-09. Duplicates the conflict in row 1 — resolve both together. **Actual record content** (read live, 2026-09-09): all 4 rules Active=true, Monitoring Period Days=30, Rule Version=1, all created by the same user (ICT Department, usrcWjDcYxE006Wgd) within a 6-minute window on 2026-09-03 — Early Warning (Threshold 2), Concern (5), Consultation (7), Red Flag (10), each with a distinct escalating Alert Message. This reads as a deliberately designed 4-tier escalation ladder, not a stray leftover — recommend leading sign-off discussion with this table rather than asking the question in the abstract. If the 4-tier model is confirmed, classification/rolling-cycle/alert-case logic must evaluate every active rule per intern per day (not one threshold), and FR-09 duplicate-prevention applies per rule/level rather than once per cycle — a real code-shape difference, not just a parameter change. |
